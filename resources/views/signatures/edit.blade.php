...
@section('title', 'QManager - Editar Assinatura')
    ...
    <section id="index-container">
       <x-dashboard.content class="md:bg-white md:dark:bg-[var(--dark-fundo-card)] md:p-5">
            <x-dashboard.title-form class="pb-3">
                <x-slot:title>Ediatr Assinatura</x-slot:title>
                <x-slot:disclaimer>
            ...
            <x-dashboard.form-container 
                method="POST" 
                action="{{ route('signatures.update', ['signature' => $signature->id]) }}"
            >
                    ...
                    <x-dashboard.input-select name="account_id" id="account_id">
                        ...
                        @foreach ($accounts as $account)
                            <option value="{{ $account->id }}" 
                                @selected(old('account_id', $signature->account_id) == $account->id)
                ...
                <x-dashboard.input-container>
                    ...
                    <x-dashboard.input-select name="plan_id" id="plan_id">
                        ...
                        @foreach ($plans as $plan)
                            <option 
                                @selected(old('plan_id', $signature->plan_id) == $plan->id)
                                value="{{ $plan->id }}"
                            >
                ...
                <x-dashboard.input-container>
                    ...
                    <x-dashboard.form-input 
                        ...
                        id="discount"
                        min="0"
                        value="{{ old('discount', $signature->discount) }}"
                    ></x-dashboard.form-input>
                ...
                <x-dashboard.input-container>
                    ...
                    <x-dashboard.form-input 
                        ...
                        name="start_date" 
                        id="start_date"
                        value="{{ old('start_date', $signature->start_date->format('Y-m-d')) }}"
                    ></x-dashboard.form-input>
