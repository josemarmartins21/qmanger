...
<body>
    <div class="page">
        <div class="card">
            ...
            <h1 class="title">Assinaturas expiradas</h1>
            ...
            <div class="notice">
                    <p><strong>Resumo rápido:</strong> as seguintes assinaturas expiraram hoje.</p>
                    <ul class="list">
                        @foreach ($signatures as $signature)
                            @if (
                            Carbon::today()->diffInDays($signature->end_date, true) == 0 
                            AND $signature->status
                            
                            )
                                <li class="list-item">
                                    <p class="item-title">{{ Account::find($signature->account_id)?->name ?? 'Conta desconhecida' }}</p>
                                    <p class="item-meta">Esta assinatura expirou hoje.</p>
                                </li>
                            @endif
                        @endforeach
                    </ul>
